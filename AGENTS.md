...
- Kalshi LIVE detection (structured progress): milestone -> `GET /live_data/batch` -> `game_progress` when `details.widget_status == "live"` (`details.status` may differ, e.g. `inprogress`)
