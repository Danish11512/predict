...
- Kalshi LIVE detection (structured progress): milestone -> `GET /live_data/batch` -> `game_progress` when batch returns usable `details` (calendar row `is_live` / sports filters gate presentation; `widget_status` not required for attaching progress)
