...
| Variable | Purpose |
|----------|---------|
| `APP_ENV` | `development` (default) enables the dev hub, request log, and OpenAPI browser UIs. `production` disables those and hides `/docs`, `/redoc`, and `/openapi.json`. |
...
Install optional dev tooling (sqladmin CRUD shell) from `backend/`:
...
Always mounted (including when `APP_ENV=production`):

| Method | Path | Description |
|--------|------|-------------|
| GET | `/health` | Liveness; includes whether Kalshi credentials are configured. |
| GET | `/portfolio/balance` | Proxies signed GET to Kalshi portfolio balance (requires API key + PEM). |
| GET | `/markets` | Signed GET to Kalshi `/markets` (optional `limit`, `cursor`, `status`). |
| GET | `/ws/smoke` | Opens Kalshi WebSocket with signed headers, subscribes to `ticker`, returns first frame or timeout note. |
| GET | `/calendar-live` | Sports LIVE calendar (v1 `card_feed` + aggregation fallback). |
...
| * | `/crud/*` | sqladmin (only if `uv sync --extra dev` installed SQLAlchemy + sqladmin). |
...
| GET | `/dev/calendar-live` | HTML table for `GET /calendar-live`. |
