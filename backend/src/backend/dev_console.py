  ...
  <main>
    ...
    <section class="hub-section" id="hub-home">
      ...
      <ul class="links">
        ...
        <li><a href="/crud">sqladmin</a> (<code>/crud</code>) — requires <code>uv sync --extra dev</code> in <code>backend/</code></li>
        <li><a href="/dev/requests">Live request log</a> (<code>/dev/requests</code>)</li>
      </ul>
      <p class="note">From the Vite app, call these via the proxy as <code>/api/…</code> (prefix stripped server-side).</p>
    </section>
    ...
    <section class="hub-section" id="hub-kalshi">
      ...
      <ul class="links">
        <li><a href="/portfolio/balance"><code>GET /portfolio/balance</code></a> — REST signing check</li>
        <li><a href="/markets"><code>GET /markets</code></a> — markets (optional query: <code>limit</code>, <code>cursor</code>, <code>status</code>)</li>
        <li><a href="/ws/smoke"><code>GET /ws/smoke</code></a> — WebSocket auth + ticker subscription (may take a few seconds)</li>
        <li><a href="/calendar-live"><code>GET /calendar-live</code></a> — sports LIVE (card_feed + aggregation fallback)</li>
        <li><a href="/dev/calendar-live">Calendar LIVE table</a> — fetches <code>/calendar-live</code></li>
      </ul>
    </section>
    <section class="hub-section" id="hub-calendar">
      <h2>Calendar LIVE (Kalshi)</h2>
      <p>Sports strip + aggregation fallback: <a href="/calendar-live"><code>GET /calendar-live</code></a>.</p>
      <ul class="links">
        <li><a href="/dev/calendar-live">Calendar LIVE table</a></li>
      </ul>
    </section>
    <section class="hub-section" id="hub-crud">
      <h2>Admin (CRUD)</h2>
      <p><strong>sqladmin</strong> mounts at <a href="/crud"><code>/crud</code></a> when <code>sqlalchemy</code> + <code>sqladmin</code> are installed (<code>uv sync --extra dev</code> from <code>backend/</code>). No models are registered yet.</p>
    </section>
...
CALENDAR_LIVE_DEV_HTML = """<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
...
<body>
  <h1>Calendar LIVE</h1>
  <p>Loads <code>/calendar-live</code> (same origin). <a href="/">Dev hub</a> · <a href="/dev/requests">Live request log</a></p>
  <div id="root"></div>
  <script>
    ...
    function refresh() {
      ...
      fetch("/calendar-live")
        .then(async (r) => {
          ...
          if (!data) {
            throw new Error("Expected JSON from /calendar-live");
          }
          return data;
        })
        .then((data) => {
          ...
          if (data.parity) {
            const ph = textEl("h2", "Parity vs aggregation top N");
...
def mount_dev_console(app: FastAPI, settings: Settings) -> None:
    ...
    @app.get("/dev/calendar-live", include_in_schema=False)
    def dev_calendar_live() -> HTMLResponse:
        return HTMLResponse(CALENDAR_LIVE_DEV_HTML)
